...
var AMT = function() {
    ...
    var STORE = "http://ls-dev.i3mainz.hs-mainz.de/rdf4j-server/repositories/amtkongeos2018";
                    ...
                    for (var j1 in graph.edges) {
                        for (var j2 in graph.edges) {
                            if (graph.edges[j1].to == graph.edges[j2].from && graph.edges[j1].role == a.antecedent1 && graph.edges[j2].role == a.antecedent2) {
                                var fromConcept = null;
                                var toConcept = null;
                                for (var k in graph.nodes) {
                                    if (graph.nodes[k].id === graph.edges[j1].from) {
                                        fromConcept =graph.edges[j1].from;
                                    }
                                    if (graph.nodes[k].id === graph.edges[j2].to) {
                                        toConcept = graph.edges[j2].to;
                                    }
                                }
                                var c = null;
    ...
    this.editAssertion = function(role, from, to, width) {
        ...
        if (index >= 0) {
            ...
            if (width > 0) {
                ...
                console.log(GRAPH.edited.edges);
            }
        }
        return consistent();
    };
